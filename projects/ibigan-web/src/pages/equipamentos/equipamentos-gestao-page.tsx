...
export function EquipamentosGestaoPage() {
  ...
  usePageToolbar({
    title: 'Dashboard',
    description: 'Dashboard do parque de equipamentos',
    actions: (
    ...
    <div className="flex min-h-0 w-full min-w-0 flex-col gap-6 pb-2">
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Estoque, alertas e rankings de uso</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {loadingResumo
          ...
          : stats.map((stat) => {
              ...
              return (
                <Card key={stat.label} className={cn('overflow-hidden', styles.card)}>
                  <CardContent className="flex gap-3 p-4">
                    <div className={cn('w-1 shrink-0 rounded-full', styles.bar)} aria-hidden />
            ...
            <>
                ...
                <ul className="mt-2 space-y-1 text-muted-foreground">
                  {(alertas?.vencidos.itens ?? []).slice(0, 5).map((item) => (
                    <li key={item.emprestimo_id}>
                      {item.patrimonio} · {item.colaborador} · {item.dias_vencido} dias
                    </li>
                  ))}
              ...
              <div>
                <p className="font-medium text-amber-700 dark:text-amber-400">
                  Próximos do vencimento ({alertas?.proximos_vencimento.total ?? 0})
                </p>
                <ul className="mt-2 space-y-1 text-muted-foreground">
                  {(alertas?.proximos_vencimento.itens ?? []).slice(0, 5).map((item) => (
                    <li key={item.emprestimo_id}>
                      {item.patrimonio} · {item.colaborador} · {item.dias_restantes} dias restantes
                    </li>
                  ))}
            ...
            {loadingRankings ? (
              ...
              <ul className="space-y-2 text-sm">
                {(rankings?.mais_utilizados ?? []).slice(0, 5).map((item) => (
                  <li key={item.patrimonio} className="flex justify-between gap-2">
                    <span>
                      {item.patrimonio} · {item.tipo}
                    </span>
                    <span className="text-muted-foreground">{item.total_dias} dias</span>
                  </li>
                ))}
              </ul>
            )}
              ...
              <ul className="space-y-2 text-sm">
                {(rankings?.mais_manutencao ?? []).slice(0, 5).map((item) => (
                  <li key={item.patrimonio} className="flex justify-between gap-2">
                    ...
                    <span className="text-muted-foreground">{item.total_manutencoes}x</span>
                  </li>
                ))}
              </ul>
            )}
